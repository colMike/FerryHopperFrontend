...
import { brands as brandsData, categories as categoriesData, products as productsData, customers as customersData, tags as tagsData, vendors as vendorsData } from 'app/mock-api/apps/ecommerce/inventory/data';
...
export class ECommerceInventoryMockApi
{
    ...
    registerHandlers(): void
    {
        ...
        // -----------------------------------------------------------------------------------------------------
        this._fuseMockApiService
            .onGet('api/apps/ecommerce/inventory/customers', 300)
            .reply(({request}) => {
                ...
                // Sort the products
                if ( sort === 'id' || sort === 'firstName' || sort === 'lastName' )
                {
                    customers.sort((a, b) => {
                        const fieldA = a[sort].toString().toUpperCase();
                        const fieldB = b[sort].toString().toUpperCase();
                ...
                {
                    // Filter the products
                    customers = customers.filter(contact => contact.name && contact.name.toLowerCase().includes(search.toLowerCase()));
                }

                // Paginate - Start
                const productsLength = customers.length;
